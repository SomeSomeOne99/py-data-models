...
temperature_adjusted = [x + (random() * 1) - 1 for x in temperature]
sarimaModel.train(temperature_adjusted, 2, 0, 1, 24)
temperature_predictions2 = sarimaModel.predict(temperature_adjusted, 24)
temperature_adjusted2 = [x + (random() * 2) - 1 for x in temperature]
sarimaModel.train(temperature_adjusted2, 2, 0, 1, 24)
temperature_predictions3 = sarimaModel.predict(temperature_adjusted2, 24)
temperature_adjusted3 = [x + (random() * 4) - 1 for x in temperature]
sarimaModel.train(temperature_adjusted3, 2, 0, 1, 24)
temperature_predictions4 = sarimaModel.predict(temperature_adjusted3, 24)
import matplotlib.pyplot as plt

# Plot temperature predictions
plt.figure(figsize=(14, 7))
plt.plot(range(len(temperature)), temperature, label='Actual Temperature')
plt.plot(range(len(temperature), len(temperature) + len(temperature_predictions)), temperature_predictions, label='Original Data', linestyle='--')
plt.plot(range(len(temperature), len(temperature) + len(temperature_predictions)), temperature_predictions2, label='Shifted Data (+/- 0.5)', linestyle='--')
plt.plot(range(len(temperature), len(temperature) + len(temperature_predictions)), temperature_predictions3, label='Shifted Data (+/- 1)', linestyle='--')
plt.plot(range(len(temperature), len(temperature) + len(temperature_predictions)), temperature_predictions4, label='Shifted Data (+/- 2)', linestyle='--')
plt.xlabel('Time')
